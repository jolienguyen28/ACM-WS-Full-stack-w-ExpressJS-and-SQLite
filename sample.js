...
app.post('/api/users', (req, res) => {
  ...
  const sql = 'INSERT INTO users (name, balance) VALUES (?, ?)';
  db.run(sql, [name, balance], function(err) {
    if (err) {
      res.status(500).send('Server error');
    } else {
...
app.put('/api/users/:id', (req, res) => {
  const { id } = req.params;
  const { name, balance } = req.body;
  const sql = 'UPDATE users SET name = ?, balance = ? WHERE id = ?';
  db.run(sql, [name, balance, id], function(err) {
    if (err) {
      res.status(500).send('Server error');
    } else {
